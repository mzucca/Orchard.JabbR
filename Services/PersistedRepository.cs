...
namespace JabbR.Services
{
    public class PersistedRepository : IJabbrRepository
    {
        ...
        {
            var original = _db.Rooms.Find(room.Key);

            if (original != null)
            {
                _db.Entry(original).CurrentValues.SetValues(room);
                _db.SaveChanges();
            }
        }
        ...
        {
            var original = _db.Users.Find(user.Key);

            if (original != null)
            {
                _db.Entry(original).CurrentValues.SetValues(user);
                _db.SaveChanges();
            }
