...
namespace JabbR.Hubs
{
    ...
    public class Chat : Hub, INotificationService
    {
        ...
        private readonly IRealtimeLogger _logger; // Realtime logger
        ...
        private readonly IQAManager _qaManager;

        private ApplicationSettings _settingsRecord;

        public Chat(
            IRecentMessageCache recentMessageCache,
            IJabbrRepository repository,
            ContentProviderProcessor contentProviderProcessor,
           ICache cache,
           IContentManager contentManager,
           IOrchardServices services,
           IRealtimeLogger logger,
           ISettingsManager chatSettingsService,
           IVirtualPathProvider virtualPathProvider,
           Orchard.Security.IAuthenticationService authenticationService,
            IQAManager qaManager)
        {
            ...
            _resourceProcessor = contentProviderProcessor;
            _cache = cache;
            //_contentManager = contentManager;
            _orchardServices = services;
            _virtualPathprovider = virtualPathProvider;
            _service = new ChatService(_cache, _recentMessageCache, _repository, chatSettingsService);
            //_service = new ChatService(_cache, _recentMessageCache, _repository, chatSettingsService, routingManager);
            _settingsRecord = chatSettingsService.Load();
            _authenticationService = authenticationService;
            _qaManager = qaManager;
    }
	...
	private string UserAgent
        ...
        public void Join()
        {
            _logger.Information("Chat.Join: Enter");
        ...
        public void Join(bool reconnecting)
        {
            ...
            _logger.Debug("Chat.Join: userId={0}",userId);

            // Try to get the user from the client state
            ChatUser user = _repository.GetUserById(userId);
            if (user == null)
            {
                _logger.Error("Chat.Join: userId={0} not found", userId);
                return;
            }
            if (reconnecting)
            {
            ...
            {
                _logger.Log("Chat.Join: {0}:{1} connected.", user.Name, Context.ConnectionId);
            ...
            _logger.Debug("Chat.Join: getClientState - ActiveRoom={0}", clientState.ActiveRoom);
        ...
        public bool Send(ClientMessage clientMessage)
        {
            ...
            if (_settingsRecord.MaxMessageLength > 0 && clientMessage.Content.Length > _settingsRecord.MaxMessageLength)
            {
                throw new HubException(String.Format(LanguageResources.SendMessageTooLong, _settingsRecord.MaxMessageLength));
            }
            ...
            if (_settingsRecord.EnableAutomaticAnswering)
            {
                Task.Run(async delegate
                {
                    int delay = 3000;
                    MessageViewModel answer = _qaManager.GetAnswer(room.Name, chatMessage.Content, out delay);
                    if (answer != null)
                    {
                        await Task.Delay(delay);
                        Clients.Group(room.Name).addMessage(answer, room.Name);
                    }
                });
            }
            return true;
        ...
        public Task<List<LobbyRoomViewModel>> GetRooms()
        {
            ...
            if (user == null)
            {
                _logger.Error("Chat.GetRooms: userId={0} not found", userId);
                return null;
            }
        ...
        public async Task LoadRooms(string[] roomNames)
        {
            ...
            {
                _logger.Error("Chat.GetRooms: userId={0} not found", userId);
                throw new ArgumentException("User " + userId
                    + "not found");
            }
        ...
        {
            _logger.Debug("Chat.Typing start on {0}", roomName);
            //string userId = Context.GetUserId();
            _logger.Debug("Chat.Typing: userId {0}", userId);

            ChatUser user = _repository.GetUserById(userId);
            if (user == null)
            {
                _logger.Error("Chat.Typing room {0}, userId:{1} NOT Found", roomName, userId);
                return;
            }
            ...
            if (room == null || (room.Private && !user.AllowedRooms.Contains(room)))
            {
                _logger.Error("Chat.Typing room {0} NOT verified for user {1}", roomName, userId);
                return;
            }
        ...
        public void UpdateActivity()
        {
            ...
            if (user == null)
            {
                _logger.Error("Chat.UpdateActivity: User {0} NOT Found", userId);
                return;
            }
            foreach (var room in user.Rooms)
            {
        ...
        private void LogOn(ChatUser user, string clientId, bool reconnecting)
        {
            ...
            _logger.Debug("Chat.LogOn user:{0}-{1} client:{2}", user.Id, user.Name, clientId);
        ...
        private string userId
        {
            get
            {
                ...
                return _authenticationService.GetAuthenticatedUser().UserName;
            }
        }
    }
}
